...
```{r}
read_excel("assets/data_geographies_v1.xlsx", sheet = "list-of-countries-etc")
```
...
```{r}
ggsave("assets/my_histogram.pdf", width = 6, height = 4)
```
...
ggplot(gdp97, aes(y = gdp, x = "")) +
    geom_violin(fill = "blue", alpha = 0.5, linetype = 0) +
    scale_y_log10()
```
...
# gdp97$gdp_pp = gdp97$gdp / gdp97$pop
...
```{r}
filter(countries, Latitude < 0)
```

2. Find the first five African countries to join the UN.

```{r}
# countries$`UN member since`
head(arrange(filter(countries, four_regions == "africa"), `UN member since`), 5)
```

3. Make a vector of OECD country names, sorted from East to West.

```{r}
pull(arrange(filter(countries, members_oecd_g77 == "oecd"), desc(Longitude)), name)
```
...
```{r}
# select(filter(co2, country == "China"), c(year, kt))

ggplot(filter(co2, country == "China"), aes(x = year, y = kt)) +
    geom_line() +
    scale_y_log10()
```
...
```{r}
dat <- arrange(co2_1997, desc(kt_pp))
head(dat, 1)
tail(dat, 1)
```
