...
countries <- read_excel("assets/data_geographies_v1.xlsx", sheet = "list-of-countries-etc")

co2 <- read_csv("assets/yearly_co2_emissions_1000_tonnes.csv")
co2 <- gather(co2, -c(country, geo), key = year, value = kt, na.rm = TRUE)
co2$year <- parse_integer(co2$year)

stats97 <- read_csv("assets/1997_stats.csv", col_names = FALSE)
...
# ! Remember to skip first (object) input when piping
co2 %>%
    filter(year == 1997) %>%
...
population <-
    read_csv("assets/population_total.csv") %>%
    gather(-c(country, geo), key = year, value = pop, na.rm = TRUE) %>%
    mutate(year = as.integer(year))

# join population and co2 tables
co2_pp <- co2 %>%
    # left_join(population[, c("pop", "year", "pop")], by = "geo")
    left_join(select(population, c("geo", "year", "pop")), by = c("geo", "year")) %>%
    drop_na() %>%
    # create t_pp column
    mutate(t_pp = kt / pop * 1000)
```
...
co2_pp <- co2_pp %>%
    left_join(select(countries, c("geo", "eight_regions")), by = c("geo"))
```
...
co2_pp %>%
    ...
    ggplot(aes(
        x = eight_regions, # map eight_regions to x-axis
        y = t_pp, # map t_pp to y-axis
        fill = eight_regions
    )) + # map eight_regions to fill colour
...
co2_pp %>%
    group_by(year) %>%
    summarise(kt_total = sum(kt)) %>%
    ggplot(aes(x = year, y = kt_total)) +
    geom_line()
```
...
co2_pp %>%
    group_by(eight_regions, year) %>%
    summarise(total_pop = sum(pop)) %>%
    ggplot(aes(x = year, y = total_pop, color = eight_regions)) +
    geom_line()
```
...
co2_pp %>%
    ...
    ggplot(aes(x = year, y = t_pp, color = country)) +
    geom_line() +
...
co2_pp %>%
    ...
    group_by(year) %>%
    summarise(mean = mean(t_pp), sd = sd(t_pp), mean_pop = mean(pop))
```
...
co2_pp %>%
    filter(
        eight_regions == "europe_west", # filter by eight_regions
        year %in% c(1997, 2014)
    ) %>% # filter by year
...
for (g in groups) {
    dat <- co2_pp %>%
        filter(
            eight_regions == g, # filter by eight_regions
            year %in% c(1997, 2014)
        ) %>%
...
```{r}
# .x means the first argument (of map function)
map(1:5, ~ .x + 100)
```
...
do_figure <- function(g) {
    co2_pp %>%
        ...
        mutate(year = factor(year)) %>%
        ggplot(aes(x = year, y = log10(t_pp), fill = year)) +
        geom_violin() +
        labs(y = "log10(tonnes CO2 per person)")

    ggsave(str_c("./assets/", g, ".png"))
}
```

```{r}
do_figure("america_south")
walk(groups, do_figure)
```
