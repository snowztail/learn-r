...
(my.data.sum <- sum(my.data))
(my.data.mean <- mean(my.data))
(my.data.median <- median(my.data))
```
...
```{r}
my.data
(my.data.unique <- unique(my.data))
(my.data.prod <- prod(my.data))
(my.data.max <- max(my.data))
(my.data.min <- min(my.data))
(my.data.range <- range(my.data))
(my.data.var <- var(my.data))
(my.data.sd <- sd(my.data))
(my.data.sample <- sample(my.data))
(my.data.sort <- sort(my.data))
(my.data.rev <- rev(my.data))
(my.data.append <- append(my.data, 1L)) # L specifies integer
```
...
numeric.vector[-1]
numeric.vector[-1:-3]
numeric.vector[seq(1, 3, 1)]
numeric.vector[seq(1, 3, 2)]
numeric.vector[seq(1, 3, 3)]
numeric.vector[seq(-1, -3, -3)]
```
...
```{r}
# Generate integers from -5 to 5 and store in variable x
(x <- -5:5)
# Find indexes of positive elements in x and store the boolean vector in variable index
(index <- x > 0)
# Sum up the elements of index
sum(index)
# Sum up the elements of x indexed by index
sum(x[index])
```
...
x <- 20:10
index.odd <- (x %% 2)
as.logical(index.odd)

index.gt15 <- (x > 15)

index.odd & index.gt15
...
```{r}
(donations <- numeric(length(days)))
donations[profit > 50] <- profit[profit > 50] - 50
donations
```
