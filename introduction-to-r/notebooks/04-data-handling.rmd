...
(density <- planets$mass / volume)

planets$density <- density
density.earth <- planets[planets$name == "Earth", "density"]
planets$relative.density.to.earth <- round(density / density.earth, 3)
...
```{r}
my.df <- read.csv("assets/states.csv", stringsAsFactors = FALSE)
my.df
```
...
```{r}
mlb <- read.csv("assets/mlb_players.csv")

# Convert position and team strings to factors
mlb$Position <- factor(mlb$Position)
mlb$Team <- factor(mlb$Team)

(records.count <- dim(mlb)[1])
(fields.count <- dim(mlb)[2])
(column.names <- names(mlb))
str(mlb)
summary(mlb)
```
...
```{r}
bmi <- function(weight.lbs, height.inches) {
    weight.kgs <- weight.lbs * 0.45359237
...
```{r}
BMI <- bmi(mlb$Weight.lbs, mlb$Height.inches)
(mlb$BMI <- BMI)
summary(mlb)
```
...
```{r}
sum(mlb$BMI > 25) / records.count * 100
```
