...
hist(pima$pregnant,
     main="Number of participants by pregnant times in the Pima dataset",
...
```{r}
# Insert your code here
plot(pima$bmi, pima$glucose, main = "Scatterplot of glucose and BMI",
     xlab = "BMI (weight in kg/(height in metres squared))",
...
```{r}
# Insert your code here
plot(pima$pregnant, pima$glucose, main = "Scatterplot of glucose and pregnant",
     xlab = "Number of times pregnant)",
     ylab = "Glucose level (mg/dL)",
     pch = 16,
     col = viridis(2)[pima$test])
...
```{r}
# Insert your code here
plot(pima$diastolic, pima$glucose, main = "Scatterplot of glucose and diastolic",
...
```{r}
# Insert your code here
plot(pima$triceps, pima$glucose, main = "Scatterplot of glucose and triceps",
...
```{r}
# Insert your code here
plot(pima$insulin, pima$glucose, main = "Scatterplot of glucose and insulin",
...
```{r}
# Insert your code here
plot(pima$diabetes, pima$glucose, main = "Scatterplot of glucose and diabetes",
...
```{r}
# Insert your code here
plot(pima$age, pima$glucose, main = "Scatterplot of glucose and age",
     xlab = "Age (years)",
     ylab = "Glucose level (mg/dL)",
     pch = 16,
     col = viridis(2)[pima$test])
...
```{r}
# Insert your code here
plot(pima$test, pima$glucose, main = "Scatterplot of glucose and test",
     xlab = "test whether the patient shows signs of diabetes (coded 0 if negative, 1 if positive)",
     ylab = "Glucose level (mg/dL)",
     pch = 16,
     col = viridis(2)[pima$test])
```
...
```
This tells us that the mean insulin level when glucose level = 0 is -118, with reasonable evidence that the interval -157 to -72 contains true intercept value (not informative biologically). For every unit increase of glucose, we
expect the individual's insulin level to increase by 2.23 with a 95% confidence interval of 1.92 and 2.55, which is a relatively large effect, with a strong evidence (p-value=8.51e-37).
...
summary(fit.glucose_bmi) # get slope (beta coefficient)
confint(fit.glucose_bmi) # get confidence interval
plot(glucose ~ bmi, data=pima,
     main = "Linear regression using glucose as dependent and bmi as independent variable",
     xlab = "BMI (weight in kg/(height in metres squared))",
     ylab = "Glucose level (mg/dL)",
     pch = 16,
     col = viridis(2)[pima$test])
abline(fit.glucose_bmi)
```
...
```{r}
# Insert your code here
fit.all <- lm(glucose ~ ., data=pima) # fit model
summary(fit.all) # get slope (beta coefficient)
confint(fit.all) # get confidence interval
plot(glucose ~ ., data=pima,
     main = "Linear regression using glucose as dependent and bmi as independent variable",
     xlab = "BMI (weight in kg/(height in metres squared))",
     ylab = "Glucose level (mg/dL)",
     pch = 16,
...
nullmod <- glm(test~1, family=binomial, pima)
1-logLik(fit.all)/logLik(nullmod)

summary(fit.all)
anova(fit.test_glucose, fit.all, test="LRT")
```
